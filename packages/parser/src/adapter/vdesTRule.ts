...
import { TplTokenRule } from "../tokenizer/tplTokenizer";

export class VdesTRule implements TplTokenRule {
    test = /{{([@#]?)[ \t]*(\/?)([\w\W]*?)[ \t]*}}/

    use(match: any, raw: string, close: string, code: string): object {
        const compiler: Compiler = this as unknown as Compiler;
        ...
        switch (key) {
            ...
            case 'each':
                group = VdesTRule._split(esTokens);
                group.shift();
