...
import { store, addFile, deleteFile, setActive } from '../../util/store'
import { ref } from 'vue'
import type { VNode } from 'vue'

const pending = ref(false)
const pendingFilename = ref('other.vdest')
...
function doneAddFile() {
  ...
  if (
    !filename.endsWith('.vdest')
  ) {
    store.errors = [`Playground only supports *.vdest`]
    return
  }
  ...
  pending.value = false
  addFile(filename)
  pendingFilename.value = 'oter.vdest'
}
</script>
